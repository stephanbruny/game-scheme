...
(define sprites (makelist (lambda () (cons (rand-pos) (rand-move)) ) 100))
(display sprites)
; (display li1)
...
(define (update-sprites spritelist)
    (map (lambda(item)
         (if (pair? item)
            (let ((pos (car item))(mov (cdr item)))
...
(define (update dt)
    (set! current-time (+ dt current-time))
    (set! sprites (update-sprites sprites))
